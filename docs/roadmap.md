...
- that same pass now makes the remaining missing owner lanes explicit under `B-007`: `EntityMap`, `Workflow`, `HierarchyRule`, and `ConvertRule` each need either honest proof or an explicit permanent boundary, while `ManagedProperty`, `Organization`, `ComplexControl`, and `CustomControlDefaultConfig` are now explicit boundaries rather than silent omissions
- with the former app-shell target-shape remainder closed honestly and the current local option-set / quick-card / saved-query-visualization slice plus owner-family inventory pass landed, later `B-007` breadth should cover those four owner lanes, the remaining non-image schema-detail gaps, and richer user-owned or otherwise unsupported view/query/visualization breadth rather than reopening settled boundaries
