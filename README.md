...
- The active later-`B-007` remainder is now narrower outside app shell: current local picklist and boolean option sets, quick/card forms, and solution-scoped saved-query visualizations are now evidence-backed through live readback and stable-overlap, while broader non-image schema-detail gaps plus user-owned or richer visualization breadth remain active.
